import tsplib95


def tsplib_distance_matrix(tsplib_file: str) -> list[list[float]]:
    """Distance matrix from a TSPLIB file

    Parameters
    ----------
    tsplib_file
        A string with the complete path of the TSPLIB file (or just its name if
        it is the in current path)

    Returns
    -------
    distance_matrix
        A list of lists with the equivalent distance matrix of the input file

    Notes
    -----
    This function can handle any file supported by the `tsplib95` lib.
    """
    tsp_problem = tsplib95.load(tsplib_file)
    flat = [tsp_problem.get_weight(*edge) for edge in tsp_problem.get_edges()]
    dim = tsp_problem.dimension
    distance_matrix = [flat[i * dim : (i + 1) * dim] for i in range(dim)]

    for i in range(dim):
        distance_matrix[i][i] = 0

    return distance_matrix
